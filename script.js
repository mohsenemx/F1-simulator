   ...
   if (e.key == "s") {
      for (let i = 0; i < 20; i++) {
         if (random(0, 100) <= drivers[i].chance) {
            if (drivers[i].speed < 60)
               drivers[i].speed = random(45, 70);
         } else {
...
function Arena() {
   for (let i = 0; i < 20; i++) {
      ...
      if (drivers[i].corner == 0 && drivers[i].line == 0 || drivers[i].line == 11 || drivers[i].line == 12) {
      ...
      if (drivers[i].corner == 0 && drivers[i].line == 1) {
         ...
         if (drivers[i].LocX <= 1200) {
            c.fillStyle = `${drivers[i].color}`;
            c.imageSmoothingEnabled = true;
            c.fillRect(drivers[i].LocX += drivers[i].speed, drivers[i].LocY, 15, 15);
      ...
      if (drivers[i].corner == 1 && drivers[i].line == 3) {
         if (drivers[i].LocX <= 1790) {
            c.fillStyle = `${drivers[i].color}`;
            c.imageSmoothingEnabled = true;
            c.fillRect(drivers[i].LocX += drivers[i].speed, drivers[i].LocY += drivers[i].speed, 15, 15);
            drivers[i].totalSpeed += drivers[i].speed;
            c.fillText(drivers[i].name, drivers[i].LocX, drivers[i].LocY - 5, 300);
         } else {
            drivers[i].corner = 2
      ...
      if (drivers[i].corner == 3 && drivers[i].line == 7) {
         if (drivers[i].LocY <= 1100) {
            c.fillStyle = `${drivers[i].color}`;
            c.imageSmoothingEnabled = true;
            c.fillRect(drivers[i].LocX, drivers[i].LocY += drivers[i].speed, 15, 15);
            drivers[i].totalSpeed += drivers[i].speed;
            c.fillText(drivers[i].name, drivers[i].LocX, drivers[i].LocY - 5, 300);
         } else {
            drivers[i].corner = 4
...
function jadvalHandler() {
   ...
   drivers.forEach((e) => {
      ...
      names.push({ teamIcon: e.teamIcon, name: nam[1].substring(0, 3).toUpperCase(), color: e.color });
   });
   //jadval.innerHTML = "";
   let j = 0;
   // console.log('aaaa')
   for (let driver of drivers) {
      ...
      driver.div.innerHTML = `<p class="pos"> ${j + 1} </p><img src="${names[j].teamIcon}" class="pic"><p class="names" style="color:${names[j].color};">${names[j].name}</p>`;
      j++;
   }
...
function speedPopUpHandler() {
   ...
   let inv = setInterval(function () {
      dSpeed = Math.ceil(driver.speed * random(85,100) * (random(10,15) * 10));
      p2.innerHTML = `<img src="${driver.driverPic}" id="driverpic"><br><img id="teamLogo" src="${driver.teamIcon}"><br><p style="font-size=10px;"> ${driver.name}</p><p style="font-size=10px;">${dSpeed} km/h | ${(driver.speed * random(1400,1600) * 15).toFixed(0)} RPM </p><p style="font-size=10px;"></p>`;
      x++;
      if (x > 30) {
         clearInterval(inv);
      }
